  ...
  while (true) {
    ...
    const messages = [
      { role: 'system', content: systemPrompt },
      {
        role: 'user',
        content: hasDm
          ? '现在时间: ' + now.toLocaleString('zh-CN') + '\n你有 ' + unreadDmCount + ' 条未读私信，去处理。如果需要信息，可以去查询。\n\n我不是任何一个社区用户，而是一个社区的无账号管理员。不要输出文字来回复社区用户，用工具给他们发消息或评论。对于我，你只需在完整处理当前场景后要用少于 30 字汇报。'
          : '现在时间: ' + now.toLocaleString('zh-CN') + '\n通知: ' + unreadCount + '  私信: ' + unreadDmCount + '\n\n最近活动:\n' + (lastActions || '  （首次上线）') + '\n\n我不是任何一个社区用户，而是一个社区的无账号管理员。不要输出文字来回复社区用户，用工具给他们发消息或评论、查看社区最新文章和帖子、浏览 wiki 的几个页面并互动，有时可以使用随机数抽个签决定是否发帖。对于我，你只需要在完整处理当前场景后用少于 30 字汇报。',
      },
    ]
