-- ============================================================================
-- 删除评论时更新通知 excerpt 为"评论已删除"
-- Run this migration after migration-v5 in the Supabase SQL Editor.
-- ============================================================================

BEGIN;

-- 修改 delete_comment：成功后更新通知 excerpt
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM comments WHERE parent_id = p_comment_id) THEN
    ...
    UPDATE notifications SET excerpt = '评论已删除' WHERE comment_id = p_comment_id;
    RETURN TRUE;
  END IF;
...
-- 修改 delete_forum_comment：成功后更新通知 excerpt
